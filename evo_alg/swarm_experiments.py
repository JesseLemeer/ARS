...
- Save JSON, CSV, and simple plots for the report
"""
...
# Important for headless experiment running.
# This prevents pygame from opening a window.
...
# Import the final 4-robot swarm implementation and patch its settings per experiment.
# This keeps the experiment runner separate from your main demo file.
swarm = importlib.import_module("evo_alg.swarm_watch")


# Original 9 goals used in the successful 4-robot demo.
GOALS_9 = [
    (-300, 100),
    (-312, 145),
    (278, -203),
...
# 12-goal set for 5-robot scalability experiment.
# The extra goals are placed in reachable/open areas, not inside closed rooms.
GOALS_12 = GOALS_9 + [
    (-120, 210),
    (40, -245),
    (250, 95),
...
# Safe start poses. First 4 match the stable swarm file; 5th is near the lower-right goal area.
START_POSES_5 = [
    (-200.0, 0.0, 0.0),
...
def available_genomes() -> list[Path]:
    """Return genome files to compare if they exist."""
    candidates = [
        BASE_DIR / "best_genome_staged.npy",
        BASE_DIR / "best_genome_latest_staged.npy",
    ]
    return [p for p in candidates if p.exists()]


def build_robots(n_robots: int, shared_map: bool, walls, landmark_groups):
    """Create robots either sharing one grid or each using its own grid."""
    ...
    # No shared map: each robot gets its own occupancy grid.
    robots = [
...
def explored_cells_metric(robots, shared_map: bool) -> tuple[int, float]:
    """Return total and average explored cells.

    For shared_map=True, all robots use the same grid, so total = shared explored cells.
    For shared_map=False, total = sum of individual explored cells.
    """
    if not robots:
        return 0, 0.0
...
def run_single_trial(
    *,
    ...
    """Run one headless swarm experiment trial."""
    ...
    # Save original module settings/functions so we can safely restore them.
    ...
    # Patch settings for this experiment.
    swarm.N_ROBOTS = n_robots
    swarm.TASK_GOALS = list(goals)
    swarm.START_POSES = START_POSES_5[:n_robots]

    # Fair goal limit:
    # 1 robot can attempt all goals; 2 robots can attempt enough goals;
    # 4 and 5 robot settings remain close to your final max=3 behaviour.
    swarm.MAX_GOALS_PER_ROBOT = max(3, math.ceil(len(goals) / n_robots))

    if not car_avoidance:
        # Disable dynamic car avoidance and physical car-car collision detection.
        # This is useful as an ablation experiment.
...
def make_experiment_plan(genomes: list[Path], max_robot_count: int) -> list[dict]:
    """Create a compact but meaningful experiment plan.

    This avoids an unnecessarily huge full factorial run while still covering:
    - robot count scaling
    - shared-map ablation
    - car-avoidance ablation
    - genome comparison
    """
    stable = next((g for g in genomes if g.name == "best_genome_staged.npy"), genomes[0])
    plans = []

    # Main scalability test with the stable genome.
    for n in [1, 2, 4]:
    ...
    # 5-robot scalability extension with 12 goals.
    ...
    # Ablation tests on the main 4-robot setup.
    ...
    # Genome comparison only on the standard 4-robot setup.
