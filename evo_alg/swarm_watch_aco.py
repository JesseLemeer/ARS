...
# Must match the goal-EA training values for the goal genomes.
MAX_V = 100.0
MAX_OMEGA = 5.0
...
# Safety recovery layer for swarm demo.
# This is NOT path planning. It only prevents a car from staying stuck forever.
RECOVERY_STEPS = 45
...
ROBOT_WARNING_DIST = 95.0       # start steering away before contact
ROBOT_SEPARATION_DIST = 62.0    # stronger avoidance zone
ROBOT_COLLISION_MARGIN = 2.0    # rectangle overlap buffer in pixels/world units
ROBOT_COLLISION_COUNT_COOLDOWN = 20  # avoid counting one overlap every frame
WALL_COLLISION_COUNT_COOLDOWN = 8    # avoid counting one wall scrape every frame
GOAL_ASSIGNMENT_SPACING = 115.0      # avoid sending two robots to nearly same goal cluster
...
# used by the goal-EA variants and are safer for the swarm demo.
START_POSES = [
    (-200.0, 0.0, 0.0),
...
class SwarmRobot:
    ...
    v: float = 0.0
    omega: float = 0.0
    collisions: int = 0              # all collision/contact events
    ...
    # Per-robot wall-follow and safety recovery state.
...
def infer_hidden_size(genome_len: int) -> int:
    """Infer hidden size for one-hidden-layer controller.

    Genome length = (inputs+1)*hidden + (hidden+1)*outputs
                  = hidden*(inputs+1+outputs) + outputs
    """
...
def goal_activations(est_x: float, est_y: float, est_theta: float, goal_x: float, goal_y: float) -> np.ndarray:
    """Same goal encoding as the goal-oriented EA files.

    The controller still receives the estimated pose, because navigation should
    use localization. Only the external success check below uses true pose.
    """
...
def assign_initial_goals(robots: list[SwarmRobot], reached_goals: set[tuple[float, float]]) -> None:
    """Assign nearest available goal to each robot using TRUE pose.

    This fixes the earlier issue where robots were initially assigned by index,
    which could send a robot to a far goal even when a nearer goal existed.
    """
    for robot in robots:
...
def assign_next_goal(robot: SwarmRobot, robots: list[SwarmRobot], reached_goals: set[tuple[float, float]]) -> None:
    ...
    Extra swarm rule:
    Do not assign two robots to goals that are very close to each other unless
    there are no other candidates. This avoids traffic jams where two cars try
    to enter the same small corridor/goal cluster.
    """

    # Workload balancing: if this robot already completed enough goals,
    # do not assign it more. Other robots can handle remaining goals.
...
def robot_robot_collision(robot: SwarmRobot, robots: list[SwarmRobot]) -> SwarmRobot | None:
    ...
    for other in robots:
        if other.robot_id == robot.robot_id:
            continue
        # Cheap distance gate before rectangle SAT.
...
def robot_pose_is_wall_safe(rx: float, ry: float, rtheta: float, walls) -> bool:
    """Return True only if the robot body is not intersecting any static wall."""
...
def push_robot_away_from_other(robot: SwarmRobot, other: SwarmRobot, walls) -> bool:
    """Safely separate two overlapping cars without pushing through walls.

    The previous push could move a car through a wall/outside the map. This
    version tries progressively smaller pushes and only accepts a pose that is
    valid with respect to static walls. If no safe push exists, it leaves the
    robot at the reverted old pose and lets recovery reverse/turn instead.
    """
    ...
    # Try away direction first, then small angular alternatives. These help in
    # corners where the direct away vector points into a wall.
    candidate_angles = [
        base_angle,
    ...
    # No safe push found. Keep the reverted old pose. Do not force a wall-crossing correction.
...
def apply_robot_robot_separation(robot: SwarmRobot, robots: list[SwarmRobot]) -> None:
    """Pre-collision avoidance: treat nearby cars as dynamic obstacles.

    Important: this function should steer/slow the car, but should not put it
    into full recovery every frame just because another car is nearby. Full
    recovery is reserved for actual overlaps or true stuck behaviour.
    """
...
def closest_obstacle_turn_sign(robot: SwarmRobot, raw_acts: np.ndarray) -> float:
    """Return turn direction away from the strongest sensor activation."""
    if raw_acts.size == 0:
...
def corner_aware_recovery_command(robot: SwarmRobot, raw_acts: np.ndarray) -> tuple[float, float]:
    """Recovery command for wall/corner/robot/stagnation contact.

    Updated logic:
    - robot collision: reverse/turn away
    - stagnation: face goal and move forward
    - wall/corner/stuck: first check if front is clear
        - if front is clear: move forward slowly
        - if front is blocked: reverse/rotate away
    """
    ...
    # Treat wall, corner, and stuck similarly because the label can switch
    # during repeated wall/corner contact.
    if robot.last_collision_type in ("wall", "corner", "stuck"):
        ...
        # If the front is clear, the robot is likely stuck by its side/corner,
        # so a slow forward motion can help it escape.
        if front_clear:
            return 30.0, turn_sign * 0.8

        # If the front is blocked, moving forward would make it worse.
        # Reverse and rotate away from the closest obstacle.
...
def update_goal_progress_state(robot: SwarmRobot, goal_dist_true: float, collision_or_recovery: bool) -> None:
    """Detect circling/no-progress behaviour.

    If the robot is not colliding but keeps moving around without reducing its
    true goal distance, trigger a short stagnation recovery. This is a local
    safety rule, not path planning.
    """
    ...
    if collision_or_recovery:
        # Existing collision recovery handles this case.
        robot.no_progress_steps = 0
        return
...
def update_corner_trap_state(robot: SwarmRobot, wall_hit: bool) -> None:
    """Detect repeated wall hits in roughly the same area and trigger longer escape."""
    if not wall_hit:
        # Slowly forget old corner history when moving normally.
...
def update_one_robot(robot: SwarmRobot, robots: list[SwarmRobot], controller, genome, walls, landmark_groups, reached_goals) -> None:
    ...
    # Sense first. Goal genome was trained with raw wall sensor activations,
    # not mapped_sensor_activations().
    ...
    if robot.recovery_steps > 0:
        ...
        if wf_cmd is not None:
            ...
            # Must match goal-EA: out[0] is directly scaled, not remapped to [0,1].
    ...
    # Save temporary pose, then check physical car-car rectangle overlap.
    ...
    if robot_hit:
        ...
        # If no safe push was possible, keep the reverted pose and let the
        # recovery command separate the cars over the next few frames.
        if not push_ok:
            robot.v = 0.0
            robot.omega = 0.0
        ...
        # Count one contact event, not one collision per frame.
        if robot.robot_contact_cooldown == 0:
    ...
    # Goal reached uses TRUE pose, as requested.
...
def draw_robot(screen, robot: SwarmRobot, colour):
    ...
    # Cyan safety circle shows the pre-collision warning range for robot-robot avoidance.
...
def draw_info_panel(screen, font, hidden, step, robots, reached_goals) -> None:
    ...
    lines = [
        ...
        ("", False),
        ("COLLISIONS", True),
        (f"Total: {total_collisions}", False),
        (f"Walls: {wall_collisions}", False),
        (f"Car-car: {robot_collisions}", False),
    ...
    for r in robots:
        ...
        lines.extend([
            ...
            (f"  pose_err={r.pose_error:.1f}", False),
